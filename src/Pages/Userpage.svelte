<script>
  ...
  //Socket connection with the backend
  const socket = io.connect("http://localhost:5000")

  //For joining a chatroom
...
<div class="background h-screen w-screen overflow-x-hidden">
  ...
  <div
    ...
    <button
      ...
      class="ml-auto flex md:hidden">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        class=" h-8 w-8 fill-slate-800 stroke-2 ">
        <path
          fill-rule="evenodd"
    ...
    <button
      on:click={() => dispatch("display", data)}
      class="ml-auto hidden w-36 items-center gap-2 rounded border-0 bg-indigo-500 py-2 px-6 text-lg text-white disabled:cursor-not-allowed disabled:bg-indigo-300 md:inline-flex">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="white"
  ...
  {#if show}
    <div class="mx-10 mt-7 flex flex-col gap-5 pb-4 md:flex-row">
      <!--Existing Chatrooms-->
        ...
        <div class="rounded-lg bg-slate-200 p-4">
          ...
          <div class="flex items-center gap-4">
            <input
              ...
              class="mt-4 mb-4  w-4/5 rounded-lg border border-gray-800 bg-slate-200 py-1 px-3 text-base leading-8 text-gray-700 outline-none transition-colors duration-200 ease-in-out focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 md:w-1/2" />
